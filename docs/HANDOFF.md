...
- `-shot`은 창을 화면 밖(-32000,-32000)에 `WS_EX_NOACTIVATE|WS_EX_TOOLWINDOW`로 띄운다. 포커스를 뺏지 않고 화면도 가리지 않으면서 GL 드로어블은 살아 있다. 숨긴 창(`SW_HIDE`)으로 하면 캡처가 검게 나오므로 그러지 말 것.
...
**맥에서 검증 못 하는 것**: ① Esc 메뉴와 실시간 입력 — `-shot`은 전진만 하고 조향을 못 해서 7 m쯤에서 홀 벽에 막힌다(그래서 미로 답파는 BFS 몫). ② **한글 글리프 렌더링** — Wine prefix에 Malgun Gothic이 없고, macOS의 AppleGothic·AppleMyungjo는 Wine이 로드하지 못한다(Impact 같은 라틴 TTF는 로드됨). 한글 자리에 폴백 폰트의 대체 글리프가 찍히는데 이건 Wine 환경 문제지 게임 버그가 아니다. 실기에서는 Malgun Gothic이 OS 기본이라 정상일 것으로 보이나, **확인 전까지는 미검증으로 둘 것**.
