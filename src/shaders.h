...
/* Metres. A rusher stands about chest high and spans a little over two;
   the first pass was twice that and filled the screen from four metres. */
"  float sc  = kind < 1.5 ? 0.24 : (kind < 2.5 ? 0.52 : 0.34);\n"
"  float leg = kind < 1.5 ? 1.30 : (kind < 2.5 ? 0.80 : 2.60);\n"
"  q /= sc;\n"
"  float d = length(q * vec3(1.0, 1.7, 0.85)) - 0.46;   // the body\n"
"  d = smin2(d, sdCap(q, vec3(0.0, 0.05, -0.30),\n"
"                        vec3(0.0, -0.02, -0.78), 0.15), 0.16);  // neck\n"
"  d = smin2(d, length(q - vec3(0.0, -0.04, -0.96)) - 0.20, 0.10);  // head\n"
"  for (int i = 0; i < 4; i++) {\n"
"    float fi = float(i);\n"
"    float a  = 0.55 + fi * 0.62;\n"
"    float ph = t * 5.5 + fi * 1.9;\n"
"    float li = 0.40 + 0.16 * sin(fi * 2.3);\n"
"    for (int sgn = 0; sgn < 2; sgn++) {\n"
"      float sx = (sgn == 0) ? 1.0 : -1.0;\n"
"      vec3 hip = vec3(sx * 0.30, 0.0, -0.42 + fi * 0.30);\n"
"      vec3 kne = hip + vec3(sx * cos(a) * 0.85 * leg, 0.72,\n"
"                            sin(a) * 0.30 * leg);\n"
"      vec3 toe = kne + vec3(sx * cos(a) * 0.80 * leg, -1.55,\n"
"                            sin(a) * 0.55 * leg + 0.16 * sin(ph));\n"
"      d = min(d, sdCap(q, hip, kne, li * 0.155));\n"
"      d = min(d, sdCap(q, kne, toe, li * 0.095));\n"
"    }\n"
"  }\n"
"  return d * sc; }\n"
...
"  if (onBeast > 0.5) {\n"
"    vec3 bc = beastKind < 1.5 ? vec3(0.30, 0.13, 0.09)\n"
"            : beastKind < 2.5 ? vec3(0.26, 0.07, 0.06)\n"
"            : vec3(0.20, 0.07, 0.14);\n"
"    float sh2 = 0.16 + 0.9 * max(dot(n, normalize(uCam - p)), 0.0);\n"
"    col = bc * sh2 * (0.5 + 1.9 * uBlink);\n"
"    col += vec3(0.9, 0.95, 1.0) * beastStun * 0.55;\n"
"    col += vec3(1.0, 0.35, 0.25)\n"
"         * pow(1.0 - max(dot(n, normalize(uCam - p)), 0.0), 3.0) * 0.30;\n"
"  }\n"
